...
class TwitterAuth
{
	...
	public function request_token()
	{
		...
		$oauth_base_text = "GET&";
		$oauth_base_text .= rawurlencode(self::URL_REQUEST_TOKEN)."&";
		$oauth_base_text .= rawurlencode("oauth_callback=".rawurlencode($this->_url_callback)."&");
		$oauth_base_text .= rawurlencode("oauth_consumer_key=".$this->_consumer_key."&");
		$oauth_base_text .= rawurlencode("oauth_nonce=".$this->_oauth['nonce']."&");
		$oauth_base_text .= rawurlencode("oauth_signature_method=HMAC-SHA1&");
		$oauth_base_text .= rawurlencode("oauth_timestamp=".$this->_oauth['timestamp']."&");
		$oauth_base_text .= rawurlencode("oauth_version=1.0");
		...
		$url .= '?oauth_callback='.rawurlencode($this->_url_callback);
		$url .= '&oauth_consumer_key='.$this->_consumer_key;
		$url .= '&oauth_nonce='.$this->_oauth['nonce'];
		$url .= '&oauth_signature='.rawurlencode($signature);
	...
	public function access_token($token, $verifier)
	{
		...
		// Т.е. сперва oauth_callback -> oauth_consumer_key -> ... -> oauth_version.
		$oauth_base_text = "GET&";
		$oauth_base_text .= rawurlencode(self::URL_ACCESS_TOKEN)."&";
		$oauth_base_text .= rawurlencode("oauth_consumer_key=".$this->_consumer_key."&");
		$oauth_base_text .= rawurlencode("oauth_nonce=".$this->_oauth['nonce']."&");
		$oauth_base_text .= rawurlencode("oauth_signature_method=HMAC-SHA1&");
		$oauth_base_text .= rawurlencode("oauth_token=".$this->_oauth['token']."&");
		$oauth_base_text .= rawurlencode("oauth_timestamp=".$this->_oauth['timestamp']."&");
		$oauth_base_text .= rawurlencode("oauth_verifier=".$this->_oauth['verifier']."&");
		$oauth_base_text .= rawurlencode("oauth_version=1.0");
		...
		@$response = file_get_contents($url);
		if($response===FALSE){
		die('Не могу подключиться к Twitter.com. <br/><br/>'.$url);
		}
	...
	public function user_data()
	{
		...
		$oauth_base_text = "GET&";
		$oauth_base_text .= rawurlencode(self::URL_ACCOUNT_DATA).'&';
		$oauth_base_text .= rawurlencode('oauth_consumer_key='.$this->_consumer_key.'&');
		$oauth_base_text .= rawurlencode('oauth_nonce='.$this->_oauth['nonce'].'&');
		$oauth_base_text .= rawurlencode('oauth_signature_method=HMAC-SHA1&');
		$oauth_base_text .= rawurlencode('oauth_timestamp='.$this->_oauth['timestamp']."&");
		$oauth_base_text .= rawurlencode('oauth_token='.$this->_oauth['token']."&");
		$oauth_base_text .= rawurlencode('oauth_version=1.0&');
		$oauth_base_text .= rawurlencode('screen_name=' . $this->_screen_name);
		...
		$url .= '&oauth_signature_method=HMAC-SHA1';
		$url .= '&oauth_timestamp=' . $this->_oauth['timestamp'];
		$url .= '&oauth_token=' . rawurlencode($this->_oauth['token']);
		$url .= '&oauth_version=1.0';
